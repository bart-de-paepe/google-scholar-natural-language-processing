...
class SearchDOIService:
    ...
    def next_step(self, link_and_media_type_and_title):
        if self.current_state.to_string() == "unprocessed":
            self.replace()
            return self.link
        if self.current_state.to_string() == "replaced":
            link = self.check_link_template()
            self.set_link(link)
            self.search_link()
            return self.link
        if self.current_state.to_string() == "link searched":
            title = link_and_media_type_and_title['title']
            self.search_crossref(title)
            return self.link
        if self.current_state.to_string() == "crossref searched":
            media_type = link_and_media_type_and_title['media_type']
            self.search_content(media_type)
            return self.link
        if self.current_state.to_string() == "content searched":
